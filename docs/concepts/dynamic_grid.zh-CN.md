...
股票模式固定从一分钟 LAST Bar 聚合 15 分钟分类，连续 8 根一致后才切换软状态。
目标仓位、撤单、网格重置和组合分配共用该有效状态；分钟 ATR、行情过期及硬风控不等待慢确认。
隔夜或缺桶后重新确认，暖机时间另计。详见[最终方案](../integrations/dynamic_grid_15m_final.zh-CN.md)。
...
仅向下跳空，且 `gap_pct > max_gap_pct`，才启动 `gap_recovery_bars` 的观察期。
`max_gap_pct = 0` 表示关闭这一跳空停买条件；恰好达到阈值不触发。

- 向上跳空：不启动新的 Gap 暂停；已有库存按原有止盈、目标和趋势规则处理。
- 向下跳空但未超过百分比阈值：不启动新的 Gap 暂停，可以继续参与符合条件的网格交易。
- 向下跳空超过百分比阈值：暂停新增买入，仍走已有库存退出和目标减仓流程。

`gap_atr` 仅保留作诊断，不再用于跳空停买。分钟 ATR 衡量的是分钟波动，
不适合直接作为隔夜跳空的独立停买尺度。例如前收盘 100、前一根分钟 ATR 为 0.20，
低开 1 美元虽达到 5 ATR，但在 `max_gap_pct = 0.08` 下不会触发 Gap 暂停。
当前配置仍保留 8% 的百分比门槛和 5 根完成 Bar 的观察期；本次不调优这些数值。
ATR 在网格间距、波动率识别和突破确认中的用途不变。

旧配置和检查点中的 `max_gap_atr_multiple` 仅作读取兼容，不再影响交易，
也不再写入新配置快照。检查点中已启动的观察期不会因升级被清零，恢复后照常递减；
真实库存、挂单和其余风险参数的对账校验不变。
